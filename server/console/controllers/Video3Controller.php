        ...
        if (file_exists($fileUrl)) {
            ...
            if ($contents == 1) {//未锁定
                ...
                foreach ($data as $key => $value1) {
                    ...
                    $fen_mp4 = '1920x1080';
                    $ffmpegFenBian = "ffmpeg -i   " . Yii::$app->params['YiiUploadUrl'] . "/videofile1/$namefix.mp4   2>&1 | perl -lane 'print $1 if /(\d{3,5}x\d{3,5})/'";
                    $c = [];
                    ...
                    for ($i = 1; $i < 11; $i++) {
                        ...
                        $ss = $start_end * $i;
                        $sql = "ffmpeg -ss $ss -i " . Yii::$app->params['YiiUploadUrl'] . "/videofile1/$namefix.mp4 -y -f image2    -t 0.001 -s $fen_mp4 $pic_url$name";
                        exec($sql);
                        $imgData[] = $name;
                    }
                    ...
                    //原始分辨率  切片
                    $ffmpeg = "ffmpeg -i " . Yii::$app->params['YiiUploadUrl'] . "/videofile1/$namefix.mp4 -vcodec copy -acodec copy -vbsf h264_mp4toannexb -threads 5 -preset ultrafast -hls_time 5 -hls_list_size 0 -hls_key_info_file " . Yii::$app->params['YiiUploadUrl'] . "/aes/$sid.keyinfo -hls_segment_filename " . Yii::$app->params['YiiUploadUrl'] . "/hls/2021/$namefix/m/v%04d.png " . Yii::$app->params['YiiUploadUrl'] . "/m3u8/$sid" . "_m.m3u8";
                    exec($ffmpeg);

                    //创建远程目录
                    $chmod = 'ssh root@8.218.16.165 "mkdir  /home/www/hls/2021/' . $namefix . '"';
                    exec($chmod);
                    //赋予权限		
                    $chmod = 'ssh root@8.218.16.165 "chmod 755 /home/www/hls/2021/' . $namefix . '"';
                    exec($chmod);

                    //粘贴数据到远程目录
                    $boscdn = "scp -r " . Yii::$app->params['YiiUploadUrl'] . "/hls/2021/$namefix/m root@8.218.16.165:/home/www/hls/2021/$namefix";
                    exec($boscdn);
                ...
                foreach ($data as $key => $value) {
                    ...
                    exec($chmod);
                    //粘贴数据到远程目录
                    $boscdn = "scp -r " . Yii::$app->params['YiiUploadUrl'] . "/hls/2021/$namefix/s root@8.218.16.165:/home/www/hls/2021/$namefix";
                    exec($boscdn);
                    //将m3u8数据写入数据库，并替换掉相关的敏感数据。以及将分片数据 。调整为oss正式地址
