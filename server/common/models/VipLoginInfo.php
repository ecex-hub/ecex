 ...
 */
class VipLoginInfo extends BaseModel {

    /**
     * @inheritdoc
     */
    public static function tableName() {
        return 't_vip_login_info';
    }
    ...
    public function getAccountInfo() {
        return $this->hasOne(AccountInfo::className(), ['uid' => 'vip_id'])->select(['uid', 'oneLevel', 'twoLevel', 'threeLevel']);
    }
     ...
     * @return type
     */
    static public function getLoginNumberAgent($date, $uid) {
        $time = strtotime($date);
        $where = [
            'and',
            ['>', 't_vip_login_info.utime', $time],
            ['<', 't_vip_login_info.utime', $time + 86400],
            [
                'or',
        ...
        $data['longin_number'] = self::find()->where($where)->joinWith(['accountInfo'])->count() ?? 0;
        return $data;
    }
     ...
     * @param type $uid
     */
    static public function loginOut($uid) {
        self::updateAll(['api_secret' => 1], ['vip_id' => $uid]);
    }
