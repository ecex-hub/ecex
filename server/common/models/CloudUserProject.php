 ...
 */
class CloudUserProject extends BaseModel {

    protected $table = 'cloud_times_api_user_project';

    public static function tableName() {
        return '{{cloud_times_api_user_project}}';
    }
        ...
        for ($i = 1; $i < 140; $i++) {
            ...
            if (empty($data)) {
                ...
                foreach ($data as $key => $value) {
                    $boor = $model->addCountryAdvanceMoney($value['user_id'], $value['machine_id'], $value['bonus_times'], $value['create_time']);
                    if (empty($boor)) {
