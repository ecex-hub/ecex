  ...
  var Controller = {
    index: function () {
      ...
      table.on('load-success.bs.table', function (e, data) {
        // 处理新的统计数据结构
        if (data.stats) {
          $('#total_money').text('¥' + data.stats.total_money);
          $('#today_money').text('¥' + data.stats.today_money);
          $('#total_users').text(data.stats.total_users + '人');
          $('#today_users').text(data.stats.today_users + '人');
          $('#completed_money').text('¥' + data.stats.completed_money);
          $('#today_completed_money').text('¥' + data.stats.today_completed_money);
        }
        
        // 保持原有的数据处理逻辑（如果有）
        if (data.buy_total !== undefined) {
          $("#buy_total").text(data.buy_total);
        }
        if (data.buy_money !== undefined) {
