 ...
 * @icon fa fa-circle-o
 */
class Bobistatic extends Backend
{
    ...
    public function index()
    {
        ...
        if (isset($customWhere[0])) {
            ...
            if (isset($item[0]) && $item[0] == 'day') {
                ...
                $signInNum = $baseQuery->where("day", '>=', $startTime)
                    ->where("day", '<=', $endTime)->sum("sign_in_num");
                $loginNum = $baseQuery->where("day", '>=', $startTime)
                    ->where("day", '<=', $endTime)->sum("login_num");
            }
        } else {
            ...
            $signInNum = $baseQuery->sum("sign_in_num");
            $loginNum = $baseQuery->sum("login_num");
        }
        $list = $this->model
            ->where($where)
            ->order($sort, $order)
        ...
        $one = $this->model->where('day', $date)->find();
        $two = $this->model->where('day', $yesterday)->find();
        ...
        $result = [
            ...
            'one_register_num' => $one->register_num ?? 0,
            'one_real_num' => $one->real_num ?? 0,
            'one_buy_product_num' => $one->buy_product_num ?? 0,
            'one_recharge_money' => $one->recharge_money ?? 0,
            'one_buy_product_money' => $one->buy_product_money ?? 0,
            'one_withdraw_money' => $one->withdraw_money ?? 0,
            'one_recharge_num' => $one->recharge_num ?? 0,
            'one_withdraw_num' => $one->withdraw_num ?? 0,
            'one_sign_in_num' => $one->sign_in_num ?? 0,
            'one_login_num' => $one->login_num ?? 0,

            'two_register_num' => $two->register_num ?? 0,
            'two_real_num' => $two->real_num ?? 0,
            'two_buy_product_num' => $two->buy_product_num ?? 0,
            'two_recharge_money' => $two->recharge_money ?? 0,
            'two_buy_product_money' => $two->buy_product_money ?? 0,
            'two_withdraw_money' => $two->withdraw_money ?? 0,
            'two_recharge_num' => $two->recharge_num ?? 0,
            'two_withdraw_num' => $two->withdraw_num ?? 0,
            'two_sign_in_num' => $two->sign_in_num ?? 0,
            'two_login_num' => $two->login_num ?? 0,
        ];
        return json($result);
    }
