...
use app\admin\model\SystemConfigure;
...
//        }
    ...
    public function login()
    {
        ...
        $url = $url ?: 'index/index';
        $sysM = new SystemConfigure();
        $info = $sysM->where("key", 5)->find();
        if ($info && $info->content) {
            $ipArr = explode(",", $info->content);
            $localIp = $this->getUserIP();
            if (!in_array($localIp, $ipArr)) {
                $this->success(__("该ip禁止访问"), "");
            }
        }
