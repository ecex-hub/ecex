...
//        self::$_instance->connect(\think\facade\Config::get('my.redis.host'),
//            \think\facade\Config::get('my.redis.port'));
//        self::$_instance->auth(\think\facade\Config::get('my.redis.password'));
        $redis = new Client([
            'scheme' => 'tcp',
            'host' => Config::get('redis.host'), // 远程 Redis 服务器地址
            'port' => 6379,  // 远程 Redis 服务器端口
            'password' => Config::get('redis.password'), // 如果 Redis 启用了密码
        ]);
        self::$_instance = $redis;
        self::$prefix = Config::get('redis.prefix');
    ...
    public function setString($key, $value, $timeout)
    {
        $key = self::$prefix . $key;
