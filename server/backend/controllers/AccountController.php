...
class AccountController extends \backend\lib\ApiBaseController
{
    ...
    public function actionBankCardBind()
    {
        $params = $this->params(['bankNameId', 'bankCard', 'realName',
            'alipay_card', 'pay_type', 'otherBankName'
        ]);
    ...
    public function actionIndex()
    {
        ...
        $system = new SystemConfigure();
        $info = $system->find()->where(['key' => 4])->one();
        if ($info) {
            $returnData['live_url'] = $info->content;
        }
        $info1 = $system->find()->where(['key' => 6])->one();
        if ($info1) {
            $returnData['official_group'] = $info1->content;
        }
        $info2 = $system->find()->where(['key' => 7])->one();
        if ($info2) {
            $returnData['chat_url'] = $info2->content;
        }
        if ($data['payPassword']) {
            $returnData['has_pay_pwd'] = true;
        }
        $this->output($returnData);
    }
    ...
    {
        $params = $this->params(['payPassword', "avatar", 'wechat', 'qq']);
        $data['utime'] = time();
        if ($params['payPassword']) {
            $data['payPassword'] = md5($params['payPassword']);
        }
        if ($params['avatar']) {
